...
const insights = defineEventHandler(async (event) => {
  ...
  try {
    ...
    console.error("Instagram Insights Error:", (error == null ? void 0 : error.message) || error);
    return getMockInsights();
  }
});
...
function generateMockMediaData() {
  ...
  const captions = [
    ...
    "Cerita hari ini: drama pagi karena sepatu hilang \u{1F605} #DailyJeds",
  ...
  return captions.map((caption, i) => ({
    ...
    timestamp: new Date(Date.now() - i * 864e5 * 3).toISOString(),
